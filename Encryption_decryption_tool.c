/* Encryption and Decryption Tool by Abhinav
 * A simple file encryption/decryption tool using offset cipher.
 * WARNING: For educational purposes only. Not for sensitive data.
 * Make a backup before using - improper use may cause data loss.
 */
...
void Encrypt_Operation(const char *file1, const char *file2)
{
    ...
    FILE *encrypted_pass_file = fopen("pass.txt", "wb");
...
/* Decrypts a file after password verification */
void Decrypt_Operation(const char *file1, const char *file2)
{
    ...
    FILE *encrypted_pass_file = fopen("pass.txt", "rb");
    if (!encrypted_pass_file)
    {
        perror("Error while reading your password from encrypted file. Maybe file was never encrypted ");
        exit(1);
    }
    ...
    if (memcmp(user_encrypted_pass, encrypted_verified_pass, size_of_verification_pass) == 0)
    {
        ...
        if (!encrypted_file || !decrypted_file)
        {
            ...
            if (decrypted_file)
            {
                fclose(decrypted_file), remove(file2);
            }
            exit(1);
        }
...
int main(int argc, char *argv[])
{
    ...
    // Choose operation based on command line argument
